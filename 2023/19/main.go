...
func splitArrBefore(a []int, b int) ([]int, []int) {
	for i, j := range a {
		if j == b {
			return a[:i+1], a[i+1:]
		}
...
func (r AcceptRule) getCategory() string { return "?" }
...
func (r RejectRule) getCategory() string { return "?" }
...
func (r GoToRule) getCategory() string { return "?" }
...
func (r DecisionRule) getCategory() string { return r.category }
...
func (r DecisionRule) execute(p RangedPart) (string, bool, RangedPart, RangedPart) {
	if r.sign == "<" {
		...
		case "x":
			...
			b, a := splitArrBefore(p.x, r.value)
		...
		case "m":
			...
			b, a := splitArrBefore(p.m, r.value)
		...
		case "a":
			...
			b, a := splitArrBefore(p.a, r.value)
		...
		case "s":
			...
			b, a := splitArrBefore(p.s, r.value)
...
func printRule(rule Rule) {
	fmt.Printf("-> %s %s %d = %s\n", rule.getCategory(), rule.getSign(), rule.getValue(), rule.getKey())
}

func printRuleMap(ruleMap map[string][]Rule) {
...
func traverseRules(currentPart RangedPart, ruleMap map[string][]Rule, key string) []RangedPart {
	...
	if rules, ok := ruleMap[key]; ok {
		...
		for _, rule := range rules {
			result, _, passedPart, failedPart = rule.execute(currentPart)
...
func solution2(rulesStr, partsStr []string) int {
	...
	for _, i := range workingParts {
		out += len(i.x) * len(i.m) * len(i.a) * len(i.s)
	}
...
func main() {
	rules, parts := readLines("input1.txt")
